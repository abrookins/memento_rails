...
class Map extends Backbone.Model
  ...
  set: (attributes) ->
    ...
    if attributes.geographic_events?
      # We shadow the map's `geographic_events` JSON data with a
      # GeographicEventList, an array of GeographicEvent models.
...
class MarkerView extends Backbone.View
  template: _.template """
    <div class='marker-content'>
      ...
      <a class='edit-marker btn' name='edit-marker' href='#markers/marker/edit/{{ id }}'>Edit</a>
    </div>
    """
  ...
  editTemplate: _.template """
    <div class='marker-edit-form'>
      ...
      <a class='save-button btn' name='save-button' href='#markers/marker/save/{{ id }}'>Save</a>
      <a class='cancel-button btn' name='cancel-button' href='#markers/marker/cancel/{{ id }}'>Cancel</a>
    </div>
    """
...
class NavigationView extends Backbone.View
  initialize: ->
    ...
    @map = @options.map
    @map.bind 'change', @loadMapSettings
  ...
  initSettingsPanel: ->
    ...
    # Hide settings panel on first load.
    $('div#settings').children().toggle()

    $('a#edit-map').click (event) =>
      event.preventDefault()
      $('div#settings').children().show()

    $('a#save-map-settings').click (event) =>
      event.preventDefault()
      @saveMapSettings()
      $('div#settings').children().hide()

    $('a#cancel-map-settings').click (event) =>
      event.preventDefault()
      # Clear the form and reset with model state.
      @loadMapSettings()
      $('div#settings').children().hide()
  ...
  loadMapSettings: =>
    title = @map.get('title')
    $('#navigation-header').text(title)
    $('#map-title').val(title)
    $('#map-location').val(@map.get('starting_location'))

  saveMapSettings: ->
    title = $('#map-title').val()
    location = $('#map-starting-location').val()
    @map.set
      title: title
